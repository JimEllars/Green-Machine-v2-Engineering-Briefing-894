...
async function syncMarketCache(env) {
  ...
  if (circuitBreaker.state === "OPEN") {
    const now = Date.now();
    if (now - circuitBreaker.last_failure < 3e5) {
  ...
  if (env.ANNY_EMAIL && env.ANNY_PASSWORD) {
    try {
      ...
      if (data?.payload?.token) {
        await env.GREEN_STATE.put("anny_session_token", data.payload.token, { expirationTtl: 518400 });
        const authTelemetry = {
...
async function fetchAnnyCombinedPortfolio(env, ctx) {
  try {
    const token = await getOrRefreshAnnySessionToken(env, ctx);
    const auth = { mode: env.ANNY_AUTH_MODE || "session-token", token };
    const headers = { "Content-Type": "application/json" };
    if (auth.token) {
      Object.assign(headers, annyAuthHeaders(auth));
    }
...
async function sendEmailItNotification(params, env) {
  ...
  try {
    ...
    if (result.success) {
      ...
      telemetry.recipients = typeof params.to === "string" ? params.to : Array.isArray(params.to) ? params.to.map((r) => r.email || r).join(", ") : "";
    }
...
var thirdweb_bridge_default = {
    ...
    if (event.cron === "* * * * *") {
      ctx.waitUntil((async () => {
        try {
          ...
          while (!listComplete) {
            const retryList = await env.GREEN_STATE.list({ prefix: "audit_retry_queue:", cursor });
    ...
    if (event.cron === "30 10 * * *") {
      ctx.waitUntil((async () => {
        try {
          ...
          const signalListExec = await env.GREEN_STATE.list({ prefix: "anny_signal_log:", limit: 1e3 });
          let totalSignals = 0;
          let buyCount = 0;
          let tpCount = 0;
          let slCount = 0;
  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && url.pathname === "/api/dlq-status") {
      ...
      if (!signature2 || signature2 !== env.AXIM_INTERNAL_KEY) {
    ...
    if (request.method === "DELETE" && url.pathname === "/api/admin/dept-summary") {
      ...
      try {
        ...
        let listComplete = false;
        while (!listComplete) {
          const listRes = await env.GREEN_STATE.list({ prefix: deptPrefix, cursor });
    ...
    if (request.method === "POST" && url.pathname === "/api/cache-sync") {
      ...
      try {
        ...
        return new Response(JSON.stringify({ error: "Failed to sync cache", details: e.message }), {
          status: 500,
          headers: { "Content-Type": "application/json", ...corsHeaders }
        });
      }
      ...
      if (action) {
        ...
        try {
          ...
          if (action === "flush_dlq") {
            ...
            let listComplete = false;
            while (!listComplete) {
              const listRes = await env.GREEN_STATE.list({ prefix: "quarantine:", cursor });
              for (const key of listRes.keys) {
                await env.GREEN_STATE.delete(key.name);
              }
              if (listRes.list_complete) {
    ...
    if (request.method === "POST" && url.pathname === "/api/admin/force-briefing-dispatch") {
      ...
      try {
        ...
        return new Response(JSON.stringify({ success: true, dispatched_at: (/* @__PURE__ */ new Date()).toISOString() }), { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } });
      } catch (e) {
    ...
    async function getSettlementTelemetry24h(env2) {
      try {
        const cacheResult = await env2.MARKET_CACHE.getWithMetadata("settlement_telemetry_24h");
        if (cacheResult.value) {
        ...
        const dbResponse = await fetch(`${env2.SUPABASE_URL}/rest/v1/blockchain_transactions?select=amount,status,created_at&status=eq.minted&created_at=gte.${new Date(Date.now() - 864e5).toISOString()}`, {
          headers: { "Authorization": `Bearer ${env2.SUPABASE_SERVICE_KEY}`, "apikey": env2.SUPABASE_SERVICE_KEY }
        });
        ...
        await env2.MARKET_CACHE.put("settlement_telemetry_24h", JSON.stringify(telemetry), { expirationTtl: 300 });
    ...
    if (request.method === "POST" && url.pathname === "/api/strategy-consult") {
      ...
      try {
        ...
        try {
          ...
          const riskTimeout = setTimeout(() => riskController.abort(), 3e3);
...
// .wrangler/tmp/bundle-QhlKEx/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default,
  middleware_miniflare3_json_error_default
];
var middleware_insertion_facade_default = thirdweb_bridge_default;
...
// .wrangler/tmp/bundle-QhlKEx/middleware-loader.entry.ts
...
export {
  ...
  fetchAnnyCombinedPortfolio,
  getOrRefreshAnnySessionToken
};
//# sourceMappingURL=thirdweb_bridge.js.map
